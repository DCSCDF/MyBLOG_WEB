<template>
        <div :class="{ ' border-gray-100 border-l-[1.6px] px-4 !mt-0 pt-4': depth > 0 }"
             class="flex gap-3">
                <!-- 头像 -->
                ...
                <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 pb-1 flex-wrap">
                                ...
                                <span v-else class="text-sm font-bold text-on-background">{{ comment.username }}</span>
                        ...
                        <a v-if="comment.website" :href="comment.website"
                           class="text-xs text-primary hover:underline mb-2 inline-block" rel="noopener noreferrer"
                           target="_blank">
                                {{ comment.website }}
                        </a>
							...
							<div v-if="showReplyForm" class="mt-6">
								...
								<div v-if="!isLoggedIn" class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
									<div>
										...
										<input v-model="localReplyForm.username" class="antd-input w-full px-3 py-2 text-sm rounded-md border-[1.6px] border-gray-200" placeholder="name" type="text"/>
									</div>
									<div>
										<label class="block text-xs font-bold text-secondary uppercase tracking-wider mb-1.5 ml-0.5">邮箱</label>
										<input v-model="localReplyForm.email" class="antd-input w-full px-3 py-2 text-sm rounded-md border-[1.6px] border-gray-200" placeholder="email@address.com" type="email"/>
									</div>
									<div>
										<label class="block text-xs font-bold text-secondary uppercase tracking-wider mb-1.5 ml-0.5">头像链接</label>
										<input v-model="localReplyForm.avatarUrl" class="antd-input w-full px-3 py-2 text-sm rounded-md border-[1.6px] border-gray-200" placeholder="https://image.url" type="url"/>
									</div>
									<div>
										<label class="block text-xs font-bold text-secondary uppercase tracking-wider mb-1.5 ml-0.5">网站链接</label>
										<input v-model="localReplyForm.website" class="antd-input w-full px-3 py-2 text-sm rounded-md border-[1.6px] border-gray-200" placeholder="https://yoursite.com" type="url"/>
									</div>
								</div>
								<!-- 已登录用户：显示简化表单 -->
								<div v-else class="mb-4">
									<div class="flex items-center gap-3 mb-4 p-3 bg-primary/5 rounded-lg">
										<img v-if="currentUser?.avatarUrl" :alt="currentUser.nickname || currentUser.username" :src="currentUser.avatarUrl" class="w-8 h-8 rounded-full object-cover"/>
										...
										<span class="text-sm font-medium text-on-background">{{ currentUser?.nickname || currentUser?.username }}</span>
								...
								<div class="mb-4">
									...
									<textarea v-model="localReplyForm.content" class="antd-input w-full p-3 text-sm min-h-[80px] rounded-md border-[1.6px] border-gray-200" placeholder="写下你的回复..."></textarea>
								</div>
								<div class="flex justify-end gap-2">
									<button class="bg-gray-200 px-6 py-2 rounded-md text-sm font-semibold hover:opacity-90 transition-opacity" @click="$emit('close-reply')">关闭</button>
									<button :disabled="submitting" class="bg-primary text-white px-6 py-2 rounded-md text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-50" @click="handleSubmit">{{ submitting ? '提交中...' : '提交回复' }}</button>
								</div>
							</div>
						</Transition>
